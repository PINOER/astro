...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const { id, lang, slug, title, relatedTool, content } = body;
    ...
    if (kv) {
      ...
      // 查找现有的配置项或创建新记录
      const existingIndex = articlesList.findIndex((item: any) => item.id === id || item.slug === slug);
      ...
      if (existingIndex > -1) {
        articlesList[existingIndex] = { ...articlesList[existingIndex], ...updatedArticleData };
      } else {
    ...
    console.error('保存文章保存接口抛出错误:', error);
    return new Response(
      JSON.stringify({
        success: false,
